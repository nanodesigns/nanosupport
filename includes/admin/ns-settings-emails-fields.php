...
function ns_notification_email_field() {
    ...
    echo '<input type="email" class="ns-field-item ns-textbox" name="nanosupport_email_settings[notification_email]" value="'. sanitize_email($email_value) .'" aria-describedby="ns-notification-mail">';
...
function ns_email_choices_field() {
    ...
    echo ns_tooltip( 'ns-new-ticket', wp_kses( __( 'If you check here, an email will be sent to Notification email notifying about every new ticket submission. Default: <code>true</code>.', 'nanosupport' ), array('code' => array()) ), 'right' );
    ...
    echo ns_tooltip( 'ns-email-response', wp_kses( __( 'If you check here, an email will be sent to the ticket author when their ticket is replied. Default: <code>true</code>.', 'nanosupport' ), array('code' => array()) ), 'right' );
    ...
    echo ns_tooltip( 'ns-agent-response', wp_kses( __( 'If you check here, an email will be sent to the agent who is assigned to the ticket when their ticket is replied. Default: <code>true</code>.', 'nanosupport' ), array('code' => array()) ), 'right' );
}
...
function ns_email_header_bg_color_field() {
    ...
    echo '<input type="text" class="ns-colorbox" name="nanosupport_email_settings[header_bg_color]" value="'. $header_bg_color .'" data-default-color="#1c5daa" aria-describedby="ns-header-bg-color">';
...
function ns_email_header_image_field() {
    ...
    echo '<input type="text" class="ns-field-item ns-textbox" name="nanosupport_email_settings[header_image]" value="'. $header_image .'" placeholder="'. esc_attr('http://path/to/image.ext') .'" aria-describedby="ns-email-header-image">';
...
function ns_email_header_text_field() {
    ...
    echo '<input type="text" class="ns-field-item ns-textbox" name="nanosupport_email_settings[header_text]" value="'. $header_text .'" placeholder="'. get_bloginfo( 'name', 'display' ) .'" aria-describedby="ns-header-text">';
...
function ns_email_footer_text_field() {
    ...
    echo '<input type="text" class="ns-field-item ns-textbox" name="nanosupport_email_settings[footer_text]" value="'. $footer_text .'" placeholder="'. $default_footer_text .'" aria-describedby="ns-footer-text">';
